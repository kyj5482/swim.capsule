...
- **2026-07-13 기준 잔액 2 크레딧.** 최저 단가(720p fast 4초)=14 → **현재 신규 생성 불가.** (미션비에호 생성으로 14 사용)
...
1. cast JSON 작성: `scripts/shorts/casts/` 기존 파일 복사 후 텍스트·음성만 교체.
   ...
   - 기록/이름은 **콘솔 LCD 하나**(`type:"dash"`)로 통일 — 좌우 부유 광고판(`board`)은
     화면과 동떨어져 보여 폐기. dash는 뱃머리에 장착된 계기판처럼 하우징+접지 그림자로
     그려지고 위치는 `keys`의 x,y(기본 540,1372) + §3a 모션 트랙을 따라 **보트와 함께
     흔들린다**(정지 아님). `keys`는 알파(페이드) 위주로 제어. 헤더 라벨(줄0)+본문.
